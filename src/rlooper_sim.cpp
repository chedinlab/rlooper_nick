...
int main(int argc, char* argv[]) {
	...
	//	sim.simulation_A();
	//}
	cout << LPR << "\nrlooper_sim.cpp::" << "main" << NN << "\n";
	cout << LPR << "\nrlooper_sim.cpp::" << "parse_argument" << NN << "\n";
	cout << YW << "\n>Running simulation: \n";
	sim.simulation_A();
...
std::string getDefaultOutfile(int argc, char* argv[], std::map<std::string, std::string>& myargs, std::map<int, std::string>& myorder, string &runScript, stringstream& debugprint) {
	...
	for (int i = 1; i < argc; i++) {
		...
		if (i == argc - 1 || (i < argc - 1 && string(argv[i+1]).substr(0, 2) == "--")) {
			...
			runScript += " --" + argkey;
			debugprint << LPR << "rlooper_sim::getDefaultOutfile:" << LBU << argkey << "=" << LGN << "True" << NN << endl;
			//ssoutfile << "_" << argkey;
		}
		else {
			...
			debugprint << LPR << "rlooper_sim::getDefaultOutfile:" << LCY << argkey << "=" << LGN << argval << NN << endl;
			if (argkey == "sigma") {
	...
	for (const auto& pair : myorder) {
		...
		if (currargs.count(argkey) > 0) {
			...
			if (argkey != "fasta" && argkey != "outname" && argkey != "outdir" && argkey != "verbose" && argkey != "debug") {
				if (currval == "True") {
					// cout << LPR << "rlooper_sim::getDefaultOutfile:" << LCY << argkey << "=" << LGN << currval << NN << endl;
			...
			cout << LPR << "rlooper_sim::getDefaultOutfile:" << LCY << argkey << "=" << LGN << currargs[argkey] << NN << " (default=" << LPR << myargs[argkey] << NN << ")" << endl;
	...
	if (!atob(myargs["outdir"])) {
		...
		cout << LPR << "rlooper_sim::getDefaultOutfile:" << LCY << "outdir" << "=" << LGN << myargs["outdir"] <<  NN << endl;
	}

	if (!atob(myargs["outname"])) {
		myargs["outname"] = getFilename(myargs["fasta"]);
		cout << LPR << "rlooper_sim::getDefaultOutfile:" << LCY << "outname" << "=" << LGN << myargs["outname"] <<  NN << endl;
	}

	
	myargs["outfile"] = myargs["outdir"] + "/" + myargs["outname"] + ssoutfile.str();
	cout << LPR << "rlooper_sim::getDefaultOutfile:" << LCY << "outfile" << "=" << LGN << myargs["outfile"] <<  NN << endl;
	
	debugprint << LPR << "rlooper_sim::getDefaultOutfile:" << LCY << "outfile_addition" << "=" << LGN << ssoutfile.str() << NN << endl;
...
void printUsage(std::string& myscript) {
	...
	// cout << "--minlength     [int]      : minimum rloop peak length in bp [50]" << endl;
	// cout << "--w/--window    [int]      : window_size for dynamic rlooper_sim [15]" << endl;
...
std::string turnSigmaIntoNiceStringForOutput(double sigma, stringstream& debugprint) { // example: -0.035 becomes min3p5 and 0.070 becomes pos7p0
	...
	debugprint << "rlooper_sim.cpp::turnSigmaIntoNiceStringForOutput:" << NN << "\n" << LGN << 
	"- sigma = " << sigma << "\n" <<
